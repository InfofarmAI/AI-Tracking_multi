...
def cc_alignment(masks, track_ids, track_id_dict, cc_alignment_dict, CcList):
    ...
    for cc_detail in cc_details:
        ...
        # 中心
        center_x = int((top_leftX + bottom_RightX) / 2)
        center_y = int((top_leftY + bottom_RightY) / 2)
...
def analyze():
    ...
    # 人物検知
    results = detector.predict(frame, device='cuda:0', classes=0, conf = 0.4)
    nd_results = results[0].cpu().numpy()
    d_bboxes = nd_results.boxes.xyxy
    d_scores = nd_results.boxes.conf
    # 2023/11/14 torisato
    ...
    if ccDetailList is not None and results[0].masks is not None:
...
if __name__ == '__main__':
    ...
    detector = YOLO('Detector\yolov8_seg\model\yolov8_seg.onnx', task='segment', verbose=False)
    tracker = MultiObjectTracker(
        tracker_name='youtureid',
        fps=5,
        use_gpu=True,
    )
    tracker.print_info()
