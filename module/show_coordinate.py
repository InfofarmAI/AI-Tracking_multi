...
import numpy as np
from utils3.Camera_conf_utils import CAMERA_AREA, camera_list
...
def open_new_window(button_texts, date_folder_path, folder_path):
    ...
    def new_button_clicked(clicked_button, camera_id):
        ...
        folder_list = get_num_folders(os.path.join(folder_path,date_folder_path), clicked_button['text'], num_folders=3)
        ...
        if first_file != None and last_file != None:
            ...
            for record in records:
                ...
                if record['chameleon_code'] == None:
                    if update_camera_id and str(update_camera_id) + "_" + TID not in tid_list:
            ...
            Popen(['python', 'start_movie.py', os.path.join(folder_path, date_folder_path), folder_list_str, formatted_date, date_folder_path, first_file, last_file, tid_list_str, str(camera_id)])
...
def create_mapping_image(records, folder_path,folder_list):
    ...
    # for id, chameleon_code,camera_id, transform_center_x, transform_center_y, log_datetime, TID ,update_camera_id in records:
    for folder in folder_list:
        ...
        for image_name in image_names_folder:
            ...
            #     if datetime.strptime(str(record['log_datetime']) if '.' in str(record['log_datetime']) else str(record['log_datetime']) + '.000', "%Y-%m-%d %H:%M:%S.%f").strftime("%H%M%S%f")[:-3] == image_name
            # ]
            matching_records = [
                record for record in records
...
if __name__ == "__main__":
    ...
    camera_create_buttons(camera_list)
