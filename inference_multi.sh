...
#推論対象の画像フォルダ
ARG4=$4
echo "Received argument: $ARG4"
#実行回数
ARG5=$5
echo "Received argument: $ARG5"

# 現在のプロセスIDを取得し、ロックファイルに保存
CURRENT_PID=$$
LOCK_FILE="/tmp/inference_pid.lock_${ARG1}"
...
OUTPUT_DIR="./${ARG1}/data/frames/${NEW_IMAGE_PATH}"
OUTPUT_DIR_GSAM2="./${ARG1}/data/gsam2_output/${NEW_IMAGE_PATH}"
#5FPS
...
if [ "$ARG5" -ne 0 ]; then
    while true; do
        gsam=$(head -n 1 "$FILE_PATH3")  # ファイルの1行目を取得

        if [ "$gsam" != "$ARG5" ]; then
            echo "条件を満たしたので5秒待機..."
            sleep 5  # 5秒待機
        else
            break  # 条件が満たされなくなったらループ終了
        fi
    done
fi
...
if [ -f "$LOCK_FILE" ]; then
  # ARG2が0でない場合にのみ処理を行う
  if [ "$ARG5" -ne 0 ]; then
    PREV_PID=$(sed -n "${ARG5}p" "$LOCK_FILE")
